# -*- coding: utf-8 -*-
"""
Created on Wed Sep 24 08:58:10 2025
...
tenure : 고객이 서비스를 이용한 총 기간(월)  1~60개월 (수치형)
frequent : 고객의 서비스 이용일   1~30일(수치형)
payment_interval : 고객의 결제 지연일  1~30일(수치형)
...
train["gender"].value_counts()/train.shape[0] #남/녀 비율
...
plt.hist(train["tenure"], bins = 30 )
#plt.scatter(train["frequent"], train["payment_interval"])
...
temp.drop("age_group",axis = 1).pivot(
    columns="support_needs", values=0).plot(kind="barh")
...
sns.countplot(
    x="contract_length", hue="support_needs",
    data=vis_df, palette="Pastel1")
