# -*- coding: utf-8 -*-
"""
Created on Wed Aug 13 14:19:54 2025
...
frame3 = frame[1:4].copy()
frame3
#0, 1, 2라는 인덱스 중 0은 존재하지 않음
# 원래는 오류를 반환
frame3.loc[0:2,:]
...
frame2["test"] = ((frame2["Year"] == 2001)  | (frame2["Year"] == 2002)) 
frame2
...
frame3.dtypes #Ohio는 문자열,Nevada는 실수
...
#New York 행과 Tow 열을 제거하시오
# Type1
data.drop(["New York"],axis="rows").drop(["Two"],axis="columns")
# Type2
data2=data.drop(["New York"],axis="rows")
data2.drop("Two", axis = "columns")

# +@ 인덱스 값을 컬럼으로 만들어보시오
data2 = data.drop("New York",axis="rows").drop(["Two"],axis="columns")
data2.reset_index()

#One 컬럼을 제외한 테이블을 만들고 컬럼별 평균을 구하시오
#drop 함수를 활용한 방식
data.drop(["One"],axis="columns").mean(axis="rows")
#변수 선택을 활용한 방식
data.loc[:,["Two","Three","Four"]]
data.loc[:,["Two","Three","Four"]].mean(axis="rows")
#차집합을 활용한 방식
selected_columns = set(data.columns) - set(["One"])
data.loc[:,list(selected_columns)]
data.loc[:,list(selected_columns)].mean(axis="rows")
