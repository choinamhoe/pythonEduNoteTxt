...
"""
결정 계수 : 모델이 학습데이터의 변동성을 얼마나 설명하는지 나타내는 수치
0.68 이라고 하면 학습데이터의 종속변수의 변동성을 68%설명한다.

수정결정계수: 결정계수를 독립변수의 개수로 나누어준 수치
컬럼 개수가 다른 회귀 모델 간의 비교에 사용
직접적으로 해석할 수 없음.
Adj. R^2 값이 0.68 이라고 해도 변수 개수로 나눠준 값이기 때문에 68% 해석한다라고 할 수 없음.

따라서 모델 비교에는 수정결정계수
모델 해석에는 결정계수를 사용해야 함

결정계수를 R^2 라고 부르는 이유는 독립변수를 1개만 고려했을 때 상관계수의 제곱값이 R^2 값이랑 동일해서 결정계수를 R^2라고 함
""" 
# 0.691
X = df[["quiz"]]
X["Intercept"] = 1
...
np.corrcoef(df["final_test"],df["quiz"])**2
