...
# 아래는 편의상 가져와서 수행
accuracy_score(checked_df["true"], checked_df["pred"]) # 0.9195
cm = confusion_matrix(checked_df["true"], checked_df["pred"]) 
pd.DataFrame(cm , index=["True", "False"], columns=["neg", "pos"])
...
# 아래는 편의상 가져와서 수행
accuracy_score(checked_df["true"], pred) # 0.8825
...
# 아래는 편의상 가져와서 수행
accuracy_score(checked_df["true"], pred) # 0.887
cm = confusion_matrix(checked_df["true"], pred) 
pd.DataFrame(cm , index=["True", "False"], columns=["neg", "pos"])
...
accuracy_score(checked_df["true"], pred) # 0.9135
cm = confusion_matrix(checked_df["true"], pred) 
pd.DataFrame(cm , index=["True", "False"], columns=["neg", "pos"])
...
# pip install pycaret numpy==1.23
