# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 10:00:32 2025
...
import pandas as pd
### 3개 파일 
files = glob.glob("E:/cjcho_work/250903/data/*csv")
pd.read_csv(files[0], encoding="cp949")

## 3개 읽어와서 하나로 합치고 df 라는 이름으로 만들기
## 10분 시작
...
df = pd.concat(
    [pd.read_csv(i, encoding="cp949") for i in files],
    ignore_index = True)
...
# 40분 시작
# 학습데이터 형태로 변환 
# 23시간 전 22시간전 21시간전 ... 현재시간 1시간후 2시간후 3시간후
dfs = []
...
concat_df.iloc[:,:24] # 23시간전 ~ 현재시간
concat_df.iloc[:,24:-1] # 1시간후 ~ 3시간후


x_index = range(24)
y_index = range(24, 27)
x = concat_df.iloc[:,x_index] # 23시간전 ~현재시간
y = concat_df.iloc[:,y_index] # 1시간후 ~ 3시간후

from sklearn.model_selection import train_test_split
# 2023/2024년도 데이터(Train data/Valid data)
tr_df = concat_df[concat_df["times"].dt.year!=2025] 
tr_df, val_df = train_test_split(
    tr_df , test_size=0.2, random_state=42)
# 2025년도 데이터(Test data)
...
# 24 분까지 구현
# 데이터와 index 집어넣으면 batch_size 에 맞게 x,y 추출하는 함수
...
outputs = tf.keras.layers.Dense(3)(x) # 1시간후~3시간후 예측
...
# 스케일링 되었던 값(-1~1)을 원래 스케일(기온범위)로 복원
