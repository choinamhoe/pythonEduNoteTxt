# -*- coding: utf-8 -*-
"""
Created on Fri Aug 29 14:24:57 2025
...
import cv2
%matplotlib auto
fruits = np.load("E:/choinamhoe/images/250829/fruits_300.npy")
fruits.shape # 흑백 이미지 300장 -> 과일

plt.imshow(fruits[0], cmap="gray")


save_path = "E:/choinamhoe/images/250829"
...
abs_diff[200:].mean()
import os
main_path = "E:/choinamhoe/images/250829/results"
...
import os
main_path = "E:/choinamhoe/images/250829/results_2"
...
# 1 미만으로 기재시 데이터를 얼마나 설명하는지를 의미 90% 설명하는
# 차원 갯수로 추출
...
pca = PCA(n_components=30)
pca.fit(fruits_2d)
transform_data = pca.transform(fruits_2d)
model = KMeans(n_clusters=3, random_state=42)
model.fit(transform_data)
label = model.labels_
import os
main_path = "E:/choinamhoe/images/250829/results_3"
for i, (img,label) in enumerate(zip(fruits,model.labels_)):
    save_path = f"{main_path}/x_{label}"
    os.makedirs(save_path, exist_ok=True)
    save_file = f"{save_path}/x_{i}.png"
