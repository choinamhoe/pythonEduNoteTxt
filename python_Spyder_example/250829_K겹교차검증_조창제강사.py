...
gs = GridSearchCV(model, xgb_parmas, n_jobs=-1, scoring="neg_mean_squared_log_error", verbose=1)
gs.fit(tr_X, tr_y)
model = gs.best_estimator_
pred = model.predict(test)
accuracy_score(test['mpg'], pred)
...
bs=BayesSearchCV(model, rf_bs_params, n_iter=30, n_jobs=-1, scoring="accuracy")
#bs.fit