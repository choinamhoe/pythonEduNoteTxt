# -*- coding: utf-8 -*-
"""
Created on Tue Aug 26 09:06:41 2025
...
"""
이산형확률분포 :
    1)베르누이 분포 : 동전던지기와 같이 한번의 실행으로
    성공 혹은 실패가 나오는 데이터가 따르는 분포
    **시행을 1번 한다.0과 1로 나타난다가 중요 부분**
    ex) 동전을 던져서 앞면이 나올 확률
    ex) 주사위를 한번 굴러서 6이 나오지 않을 확률
"""
def bern_f(p, x):
    """
    베르누이 확률함수
...
#누적 밀도함수는 확률질량함수들 더한 값
#누적밀도함수(cdf) : pmf 확률질량함수를 0부터 n까지 합한값 
#즉.rv.pmf(0) + rv.pmf(1)+...+rv.pmf(n)
...
ax.plot(range(10),probs, color="red", label = "binum(10,0.3)")

#n = 10, p= 0.5
rv=stats.binom(10,0.5)
probs = rv.pmf(range(10))
ax.plot(range(10),probs, color="green", label = "binum(10,0.5)")

#n = 10, p= 0.7
rv=stats.binom(10,0.7)
probs = rv.pmf(range(10))
ax.plot(range(10),probs, color="blue", label = "binum(10,0.7)")
ax.legend()
...
for prob,color in zip([0.3,0.5,0.7],["red","green","blue"]):
    ...
    ax.plot(range(n),probs, color=color, label = f"binum({n},{prob})")
...
#한 시간에 평균 10번 사건이 발생, 15번이 발생할 확률
fun(10, 15)

#분포에 따른 평균 분산 계산(p 114, p 172) 이산, 연속 확률 변수
#45분까지 자유롭게 읽어보기