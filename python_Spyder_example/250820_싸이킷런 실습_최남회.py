# -*- coding: utf-8 -*-
"""
Created on Wed Aug 20 15:19:16 2025
...
Index(['sepal length (cm)', 'sepal width (cm)', 'petal length (cm)',
      ...
      dtype='object')
"""
#이렇게 데이터를 쪼개는 목적은 AI모델이 강건(Robust)한 성능을 보일 수 있게
#하려고 하는것
#강건하다는 말은 데이터에 어느정도 노이즈가 있어도 틀리지 않고 잘 맞춘다는
#의미
...
valid, test = train_test_split(
    df, test_size=0.5, random_state=42, stratify=df["target"],
    shuffle=True)
...
mu
sigma
train["sepal length (cm)"]
standard_values = (
    train["sepal length (cm)"] - mu)/sigma
