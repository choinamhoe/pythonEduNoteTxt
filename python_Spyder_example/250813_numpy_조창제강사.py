# -*- coding: utf-8 -*-
"""
Created on Wed Aug 13 09:32:12 2025

@author: human
"""
# pip install numpy 
import numpy as np # as 는 너무 길어서 줄여서 사용하고 싶을 때 기재
# import numpy
# import pandas as pd
np.arange(1000000)
np.arange(1_000_000)
...
# 시간 비교 (결론 넘파이가 빨라서 써야한다)
my_arr = np.arange(100000)
my_list = list(my_arr)
%timeit  my_arr2 = my_arr * 2
%timeit my_list2 = [i*2 for i in my_list]

# shape 을 통해서 구조를 확인할 수 있다.
...
arr2 + arr3 # 마지막 shape 이 같으면 연산이 일부 가능
arr1 + arr2  # shape 이 다르면 연산이 안됨
...
# 브로드 케스팅(복사한 값이 수정되었을 때 원본에 해당되는 값이 같이 변경되는 현상)
arr = np.arange(10)
arr[5]
arr[5:8] = 12
arr

arr_slice = arr[5:8] # 복사한 값
arr_slice[1] =12345 # 복사한 값을 수정
arr_slice
arr # 원본 값도 수정된 것이 확인됨
"""
arr 의 출력
array([    0,     1,     2,     3,     4,    12, 12345,    12,     8,
           9])
"""
# 브로드케스팅 막으려면 copy() 함수로 복사해와서 사용해야 함
arr = np.arange(10)
arr[5]
arr[5:8] = 12
arr_slice = arr[5:8].copy()
arr
""" 원본 값이 변하지 않은 것을 확인 가능
...
# 홀수 행에 짝수 열 선택해서 0 으로 변경
arr[::2]
arr[::2,1::2] = 0
# 변경된 값에서 (3,2) (5,2)를 8로 변경 ( 파이썬이니까 (2,1), (4,1))
arr[[2,4],1] = 8
arr
...
arr.T
# 내적 
# arr.T[0,:] * arr[:,0] = 0, 25, 100 => 더해서 125 내적에 [0,0] 값
# arr.T[0,:] * arr[:,1] = 0, 30, 110 => 더해서 140 내적에 [0,1] 값
np.dot(arr.T , arr)
arr.T@arr
# 표준정규분포(평균 = 0 표준편차 = 1) 4x4 행렬로 16개 랜덤 생성
...
np.floor(arr) # 버림
np.round(arr) # 정수 반올림
np.round(arr,1) # 소수 n 째 자리까지 반올림

np.cos(90) 
# 코사인 함수는 라디안 값을 입력 받는데 저희 가 알고 있는 90은 각도
# 라디안 값으로 변경이 필요 np.deg2rad 활용해주거나 np.pi/180을 곱해주어야 함
np.cos(np.pi/180*90) # 각도 np.pi/180 을 곱해줘야 된다.
...
arr = np.load("D:/test.npy")


np.linalg.inv
np.linalg.qr # 행렬 분해

# M x N(SVD, QR) = M x k  , k x N
# M x M(고유값 분해, LU 분해)
"""
TMI
주성분 분석 : 
...
for _ in range(nsteps):
    # np.random.randint(2, size=1)  :0 혹은 1 랜덤하게 1개 나오게 
...
# 이미지 n장을 불러와서 n, 512, 512, 3 의 형태로 만들고자 할 때 Type1
...
# 이미지 n장을 불러와서 n, 512, 512, 3 의 형태로 만들고자 할 때 Type2
img1.shape
