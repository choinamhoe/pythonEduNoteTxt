...
t 분포:
    ...
    모분산을 모르는 상태에서 평균을 검정할 필요가 생겨서 만들어짐
t = Z/ sqrt(Y/n)
분자는 평균의 카이를 표준화 한 값
분모는 분산 개념으로 표준화 한 값
...
특징: 
    ...
    3. 자유도가 높아질수록 표준정규분포에 근사(가까워 진다)
"""
...
"""
# 표준 정규 분포와 자유도에 따른 t 분포가 얼마나 유사한지 시각화 해서 본 내용
rv1 = stats.norm(0, 1)
...
rv3 = stats.t(100)
ys3 = rv3.pdf(xs1)
plt.plot(xs1, ys3+0.1, label = "t(100)")
plt.legend()

"""
카이제곱 분포: 자료가 n 개 있을 때 분산들이 따르는 분포
F 분포: 두 그룹이 존재하고 각각의 그룹의 
    카이제곱 분포의 분산에 대한 비율
...
"""
# 람다가 3인 포아송 분포 따르는 데이터 1만개에 대한 평균 n 세트는
# 표본평균이 정규분포를 따른다.
rv = stats.poisson(3)
n = 1000
sample = 10000
...
xs = np.linspace(rv2.isf(0.999), rv2.isf(0.001), 100)
ys = rv2.pdf(xs)
