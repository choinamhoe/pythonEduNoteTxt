# -*- coding: utf-8 -*-
"""
Created on Tue Aug 26 14:57:18 2025
...
scores
mu = df.mean().values #모평균
std = df.std(ddof=0).values #모분산
mu
std
...
for i in range(5):
    ...
    print(f"{i+1} 번째 표본 평균 : {s_mean:0.3f}")
...
불편 : 편향의 다른 의미, 데이터가 치우쳐지지 않았다.
    중심 경향 척도
"""
#표본 평균의 평균 혹은 표본 평균에 기대값은 불편 추정량
np.mean(sample_means)

#모평균 값이랑 표본평균 값이 같을 수록 좋은 값
#모수의 값이랑 통계량 값이 동일할수록 좋은 값
"""
표본평균의 기대값은 불편 추정량이면서 일치 추정량이라 할 수 있음
"""
np.mean(sample_means)

"""
점추정 - 어떤 모수가 좋은 모수인가
모수가 어느 범위까지 신뢰할 수 있는가 -> 구간추정
"""
"""
점추정 결과 > 표본평균의 기대값
"""
...
표준 정규분포 형태를 띰
#유의수준 0.05 -> 95% 통계적으로 신뢰하겠다.
P(z_0.975 <= (x_bar - mu)/np.sqrt(sigma^2/n)<=z_0.025) = 0.95
영역 => 신뢰구간
...
z_0.975 부터 z.0.025까지 통계적으로 신뢰하겠다.
...
rv.isf(0.975) # 기각역 수치값이 -1.9599
"""
95% 신뢰하겠다 -> 유의수준 0.05
1종 오류 : 귀무가설이 참인데 귀무가설을 기각하는 경우 
(MU가 130인데 130이 아니라 하는 경우)
2종 오류 : 대립가설이 참인데 귀무가설을 채택하는 경우
(MU가 130가 아닌데 130이라고 하는 경우)
이해해 두는 것이 좋음
"""
