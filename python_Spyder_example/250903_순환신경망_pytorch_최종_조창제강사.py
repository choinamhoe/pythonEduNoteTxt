# -*- coding: utf-8 -*-
"""
Created on Wed Sep  3 12:46:10 2025
...
file_dir = "E:/cjcho_work/250903/data"
# 전체 데이터 읽어와서 하나로 합치기 14분 시작
# 3개 파일 읽어와서 df 라는 이름으로 합치기
# Hint: pd.concat([], axis=0)
...
df.shape
pd.merge(df, time_df, on="일시", how="right").shape
pd.merge(df, time_df, on="일시", how="left").shape
pd.merge(df, time_df, on="일시", how="outer").shape
pd.merge(df, time_df, on="일시", how="inner").shape
...
# 35분 진행
np.where(tot_df["temp"].isna()) # [8152, 8153]
...
train_form_df = train_form_df.dropna().reset_index(drop=True)
### 정리하고 26분 진행6. 2023, 2024년도 Train데이터와 validation 데이터로 만들기
...
## 남은 시간동안 시간단위 데이터를 일단위 데이터로 바꿔서 시각화 해보기
